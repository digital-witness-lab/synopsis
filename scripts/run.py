...
with open('test.txt.vecc', 'r') as f:
...
eps3_noised_embeddings_five = []

five_noise_stddev_eps3 = sigma(3, 2**(-40), 1, 1/(np.sqrt(512)))
five_noise_eps3 = np.random.normal(0, five_noise_stddev_eps3, 512)

for i in range(len(scaled_embeddings)):

    five_noise_embed_eps3 = transformed_five[i] + five_noise_eps3
    eps3_noised_embeddings_five.append(five_noise_embed_eps3)
...
for i in range(len(embeddings)):
    ...
    noised_five_dist_eps3 = distance.euclidean(transformed_five[0], eps3_noised_embeddings_five[i])
    noised_five_dists_eps3.append(noised_five_dist_eps3**2)
    

df = pd.DataFrame(data=np.column_stack((ground_truths, scaled_dists_grounds, ep_fives, noised_five_dists_eps3)),
                  columns=['ground_truth', 'ground_truth_scaled', 'five_transform', 	        'five_noised_eps3'])
...
eps3_five = pd.DataFrame(eps3_noised_embeddings_five)
eps3_five.to_csv('database.tsv', sep = '\t', index = None, header = False)
