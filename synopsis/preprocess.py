...
def euclidean(A: np.typing.ArrayLike, B: np.typing.ArrayLike):
    """
...
for i in range(len(embeddings)):
    ...
    ground_truth = euclidean(embeddings[0], embeddings[i])

    scaled_dists_ground = euclidean(scaled_embeddings[0], scaled_embeddings[i])
...
transformer_five = random_projection.GaussianRandomProjection(
    n_components=512, eps=0.38
)
...
for i in range(len(embeddings)):
    ep_five = euclidean(transformed_five[0], transformed_five[i])
...
def sigma(epsilon, delta, c=1, w=1):
    return (
        c * w * np.sqrt(2 * (np.log(1 / (2 * delta)) + epsilon)) / epsilon
    )  # log is ln
...
for i in range(len(embeddings)):
    ...
    noised_five_dist_eps6 = euclidean(
        transformed_five[0], eps6_noised_embeddings_five[i]
    )
...
df = pd.DataFrame(
    data=np.column_stack(
        (ground_truths, scaled_dists_grounds, ep_fives, noised_five_dists_eps6)
    ),
    columns=[
        "ground_truth",
        "ground_truth_scaled",
